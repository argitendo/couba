...
function ModalForm({ toggleExit }) {
  ...
  const validateForm = () => {
    ...
    // Phone Number validation (basic check for length and numeric characters)
    if (!phoneNumber.trim()) {
      ...
      <section className='bg-mainColor-tertiary translate-x-2 translate-y-2 rounded-2xl'>
        <section className='flex flex-col md:flex-row relative bg-white -translate-x-2 -translate-y-2 rounded-2xl justify-evenly'>
          ...
          <form
            ...
            <Input.MainForm
              values={fullName}
              onChanges={(e) => setFullName(e.target.value)}
              placeholders={'eg: John Doe'}
              types='text'
            >
              Full Name
            </Input.MainForm>
            ...
            <Input.MainForm
              values={phoneNumber}
              onChanges={(e) => setPhoneNumber(e.target.value)}
              placeholders={'eg: 543210987'}
              types='tel'
            >
              Phone Number
            </Input.MainForm>
            ...
            <Input.MainForm
              values={brand}
              onChanges={(e) => setBrand(e.target.value)}
              placeholders={'eg: Your Brand'}
              types='text'
            >
              Brand
            </Input.MainForm>
